...
- **Contributions (LIVE):** a "Suggest an edit or addition" modal (`src/contribute.js` + `.css`) on the map and every report/index, opened by any `[data-contribute]` trigger (reports pass `data-country`). Posts JSON (`text/plain`, `no-cors`) to a Google Apps Script (`apps-script/contributions.gs`) that appends to a review Sheet; corrections applied by hand, nothing auto-publishes. Gated by `VITE_CONTRIBUTE_URL`. FULLY WIRED 2026-08-15: Sheet "Femtech map contributions" + Apps Script web app deployed on **gkenneth2@gmail.com** (Execute as: Me, Access: Anyone), `VITE_CONTRIBUTE_URL` set in Vercel prod, buttons live, verified end-to-end (rows land in the Sheet's auto-created "Contributions" tab). To review: filter that tab by Status = New. Setup for reference: `docs/contributions-setup.md`.
- **Live visitor feed:** `/live?key=<LIVE_PAGE_KEY>` streams real-time visits (arrivals, page views, coarse location, no cookies), ported from gigikenneth.com. `src/ping.js` pings `api/ping.js` on each page load; `api/ping.js` forwards Vercel's cookieless geo headers to a secret ntfy topic; `api/live.js` (rewritten from `/live` in `vercel.json`) renders the key-gated page that subscribes to that topic. Phone push = subscribe the ntfy app to the topic. Config lives in Vercel env vars `VISITOR_NTFY_TOPIC` (secret) + `LIVE_PAGE_KEY` (page access); both no-op if unset. No DB, ntfy is the transport + "last few hours" buffer.
