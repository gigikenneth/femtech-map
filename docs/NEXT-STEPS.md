...
- **Contributions:** a "Suggest an edit or addition" modal (`src/contribute.js` + `.css`) on the map and every report/index, opened by any `[data-contribute]` trigger (reports pass `data-country`). Posts JSON (`text/plain`, `no-cors`) to a Google Apps Script (`apps-script/contributions.gs`) that appends to a review Sheet; corrections applied by hand, nothing auto-publishes. Gated by `VITE_CONTRIBUTE_URL` (buttons hidden until set). Setup: `docs/contributions-setup.md`.
